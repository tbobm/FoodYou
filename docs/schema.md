...
Single Room database: `MymymealDatabase`
(`app/src/commonMain/kotlin/dev/tbobm/mymymeal/app/app/infrastructure/room/MymymealDatabase.kt`).
`exportSchema = true`; every version's JSON schema is checked in under
`app/schemas/dev.tbobm.mymymeal.app.app.infrastructure.room.MymymealDatabase/{1..34}.json`.
...
(`app/src/commonMain/kotlin/dev/tbobm/mymymeal/app/fooddiary/infrastructure/repository/RoomFoodDiaryEntryRepository.kt`)
...
All entities live under `app/src/commonMain/kotlin/dev/tbobm/mymymeal/app/`, in
...
Defined across `MymymealDatabase.kt` (autoMigrations list + `migrations` companion list) and
`app/src/commonMain/kotlin/dev/tbobm/mymymeal/app/app/infrastructure/room/migration/`
...
| 12→18 | Auto | Incremental (isLiquid, note columns per inline doc comments in `MymymealDatabase.kt`). |
